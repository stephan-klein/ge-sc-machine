...
import traceback  # added for timestamp
...
def main(argv):
    ...
    try:
        ...
        if os.path.exists('output.json'):
            ...
            logger.info(f"[{datetime.now()}] Deleted existing output.json file")
        ...
        if cfg_graph is None:
            raise ValueError(f"Failed to generate CFG for {sm_path}")
        cg_graph = generate_cg(sm_path)
        if cg_graph is None:
            raise ValueError(f"Failed to generate CG for {sm_path}")
    ...
    # Output the results to stdout and to a file
    response_json = json.dumps(response)
    print(f"Result: {response_json}")
    
    # Also write the results to output.json file
    with open(OUTPUT_JSON, 'w') as outfile:
        outfile.write(response_json)
        logger.info(f"[{datetime.now()}] Results written to output.json")
